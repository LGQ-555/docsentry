...
| 平均 chunk 尺寸（实现完成、修复轮之后实测） | `fixed` 974 · `semantic` 1034 · `structural` 805 |
...
⚠️ **均值最差对实测 1.29×**（semantic 1034 / structural 805）。规划期测得 **1.34×**
...
1.34× → 1.29× 的收窄来自 Task 12b 给 `semantic` 加上了同一条上限；**判据不是因为收窄才改的**，
改判据的依据是"比值是差代理"那段论证。
...
**The spread is 1.29x as measured now (2026-09-24, after Task 12b), which is
...
been adopted at 1.34x, at 1.29x, or at 1.05x. Recorded here because a criterion
that was swapped while the number moved is exactly the kind of thing a reader
should be able to check, and a reader who only sees today's 1.29x would
...
def test_strategies_are_within_the_fairness_band(documents, config):
    ...
    The worst pair is **1.29x** (semantic 1034 vs structural 805). An earlier
...
| 同上 | §2 决策 1/3/4、§3.1、§3.3、§6.2、§6.3 | 均值 974/**1034**/**805**、最差对 **1.29×**（保留 1.34× 的规划期记录）、索引总量 1.24×/1.08×/1.06×、`build-client.md` 标题 **127**、补决策 2 的 Task 12b 扩充 |
