...
**实测（2026-09-24，MDX 归一化之后）三个策略的均值是 973 / 1072 / 802**，全部落在
`target_size` 的 ±25% 区间内，§6.3 的公平性约束成立。最差对 semantic/structural = **1.34×**，
成因见 §6.2 的修正说明——是决策 3 的后果，不是缺陷，但必须披露。
...
> 实测最差对是 **1.34×**（semantic 1072 / structural 802）——一个没有任何东西出错却不及格的数。
...
> 区间直接陈述要求本身：每个均值都在 `target_size` 的 ±25% 内。实测 973 / 1072 / 802，全部在 750–1250 内。
>
