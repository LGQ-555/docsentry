...
>
> **2026-09-24 实现完成后重测（MDX 归一化、决策 2 的上限、以及"噪声丢弃是系统级规则"三者均已落地）。**
> 三策略实测：
>
> | 策略 | chunk 数 | 丢弃 | 均值 | 中位数 | 总索引字符数 | 相对语料 |
> |---|---|---|---|---|---|---|
> | `fixed` | 17,940 | 1 | 974 | 1000 | 17,465,281 | 1.24× |
> | `semantic` | 14,651 | 31 | 1034 | 911 | 15,154,449 | 1.08× |
> | `structural` | 18,442 | 96 | 805 | 642 | 14,840,156 | 1.06× |
>
> 语料 774 篇 / 14,049,846 字符。均值最差对 **1.29×**（semantic / structural），三策略全部落在
> `target_size` 的 ±25% 区间内。复现：`uv run scripts/check_fairness.py`；验收：`uv run pytest -m corpus`。
>
> **"丢弃"列与两个基线的数字都是同一件事的产物**：噪声规则（决策 3）在 fix pass 之前只有
> `structural` 执行，于是同一条 tag-only 正文，`structural` 丢掉而另两个照收。提升为系统级规则后
> `semantic` 少 31 个 chunk、`fixed` 少 1 个，两者的均值随之从 973 / 1032 移到 974 / 1034（本表即修复轮之后的终值）。
> 详见决策 3 的落地补充与 §6.2。
>
...
>
> **（后续的修复轮把噪声丢弃提升为系统级规则，`semantic` 又少了 31 个 chunk，
> 落到 14,651 / 均值 1034。本表与 §6.2 的数字是那个终值；此处保留 Task 12b 落地当时的
> 观测，因为它是这个决定被做出时看到的证据。）**
...
**实测（实现完成、修复轮之后）三个策略的均值是 974 / 1034 / 805**，全部落在
`target_size` 的 ±25% 区间内，§6.3 的公平性约束成立。最差对 semantic/structural = **1.29×**
...
> **2026-09-24 修复轮补充：这条规则同样是系统级的，不是 `structural` 的特性。**
> 代码评审发现 `is_noise` 只被 `structural` 调用，实测有 **31 个 `semantic` chunk 与 1 个 `fixed`
> chunk** 剥掉标签后什么都不剩却仍被保留——它们与 `max_atomic_size` 要消灭的是同一类东西：
> 塞不进 embedding 的内容。决策 2 的先例（上限从 `structural` 特性提升为系统规则）同样适用，故三个策略
> 现在都丢弃噪声，并在公平性报告里各有一列 `dropped`（实测 1 / 31 / 96）。
>
> **一条随之而来的契约变化，必须说清**：`fixed` 的窗口若整个是噪声，丢掉它就在该文档的字符覆盖上
> **留下一个洞**——"fixed 的窗口并集覆盖每个字符"不再成立。这是有意的（不可嵌入的 chunk 不值得进索引），
> 但它意味着 `fixed` 的覆盖率不再等于 100%，报告里不能再用覆盖完整性去论证什么。
...
max_atomic_size: 4000    # 决策 2，系统级上限
max_code_size: 3000      # §6.3.3 step 6
```

**没有 `strategies` 键**（实现期修正）。本文件初稿画过一条 `strategies: [fixed, semantic, structural]`，
实现时否决了：策略列表的唯一真相源是 `chunking/registry.py`，在 yaml 里再放一份就是第二个真相源，
而 `ChunkingConfig` 是 `extra="ignore"` 的 pydantic 模型——**它会静默忽略这个键**，
于是配置看起来"生效了"而实际没有任何代码读它。要改策略集合就改注册表。

**加载期校验**（实现轮补充）：四个参数都带边界（`overlap >= 0`，其余 `> 0`），另校验
`overlap < target_size`（否则固定窗口不前进）与 `target_size <= max_atomic_size`（否则两条尺寸规则矛盾——
见 §7 的对应行）。边界不是形式主义：评审实测 `ChunkingConfig(target_size=1000, overlap=-500)`
会被接受，步长变成 1,500，一篇 2,500 字符的文档里 `[1000:1500]` 那段**进不了任何 chunk**，
而尺寸看起来一切正常。
...
> 区间直接陈述要求本身：每个均值都在 `target_size` 的 ±25% 内。实测 974 / 1034 / 805，全部在 750–1250 内。
>
> **1.34× 仍然要如实公布**——诚实的披露是"1.34×，原因是……"，不是把阈值调到刚好过关。
>
> **当前最差对是 1.29×，已经低于原来那个 1.30 的门槛——但判据不是因为这样才改的。**
> 改判据的依据是上面那段论证；在 1.34×、1.29×、1.05× 下都会改。这句必须写在这里：
> 一个判据在数字变动的同时被换掉，正是读文档的人有权核查的事；只看到今天 1.29× 的读者，
> 否则会得出"那个门槛从没被越过"的错误结论。（1.34× → 1.29× 的收窄来自 Task 12b 给
> `semantic` 加上了同一条上限。）
...
| 两条尺寸规则在配置里互相矛盾 | **在加载期拒绝**：`ChunkingConfig` 校验 `overlap < target_size` 与 `target_size <= max_atomic_size`。不在切片时选赢家——那正是 `fixed` 曾在该配置下产出 8,000 字符窗口、而另两个封顶 4,000 的成因，三个策略在悄悄度量三件不同的事 |

### 7.1 已知限制：解析失败的回退路径未计数

`StructuralChunker.chunk()` 捕获 `Exception` 后退回段落累积并 `warnings.warn`（§6.3.3 的降级路径，
实测在 774 篇上 **0 次触发**——markdown-it 是全覆盖解析器，畸形 Markdown 不抛异常，所以这条是真正的安全网）。

**它的可见性是不完整的。** 告警会为每篇受影响的文档打印一次（消息里带 `doc.url`），但：

- 没有计数器，报告里看不到发生过几次；
- 产物 chunk 上**没有任何标记**。

后果是：一篇因解析失败而**丢掉整个标题结构**的文档，在索引里与一篇本来就没有标题的文档
**无法区分**。这与决策 2 的处理恰好相反——那边切断原子单元时会打 `split_atomic` 标记，
因为"我做不到"必须留下痕迹。这里是本项目"不静默"原则**唯一未被完全兑现的地方**，
明确记为已知限制而非静默带过。修法（`last_fallback` 计数并进报告，或给 chunk 打标记）留待 M3 或之后。
