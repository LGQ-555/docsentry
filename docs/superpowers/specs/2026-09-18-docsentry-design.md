...
> **1.25×**，而 `semantic` / `structural` 切在天然边界上、无重叠，是 **1.00×**。**这个差异不消除，但必须披露。**
...
> 而 MDX 归一化之后的实测均值是 973 / 1072 / 802（fixed / semantic / structural），
...
> **2026-09-24 实现期修正（判据从"两两比值 ≤ 1.30"改为区间）**：实测最差对是 **1.34×**
...
> **1.34× 仍如实公布**，不是把阈值调到过关。详见 [M2 设计文档](2026-09-24-m2-chunking-design.md) §6.2。
